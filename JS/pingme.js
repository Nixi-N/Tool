...
@Author：怎么肥事
@Modified: Nixi-N (兼容 Surge/Loon/QX 完整修复版)
*/
...
if (typeof $request !== 'undefined' && $request) {
  ...
  if (!raw) {
    $.msg($.name, '⚠️ 未抓到参数', '请先打开 PingMe 触发一次余额查询');
    $.done();
  } else {
    let capture;
    try { 
      ...
      $.msg($.name, '⚠️ 参数损坏', '请重新打开 PingMe 抓包');
      $.done();
    }
...
function Env(name) {
  ...
  this.setdata = (v, k) => this.isQX ? $prefs.setValueForKey(v, k) : (this.isLoon || this.isSurge ? $persistentStore.write(v, k) : null);
  this.getdata = (k) => this.isQX ? $prefs.valueForKey(k) : (this.isLoon || this.isSurge ? $persistentStore.read(k) : null);
